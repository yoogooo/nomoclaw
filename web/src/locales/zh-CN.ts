const zhCN = {
  ...
  chat: {
    ...
    runtime: {
      ...
      approvalRiskHintSensitiveRead: "安全提示：检测到敏感信息路径读取，可能导致密钥或凭据泄露。请确认是否继续。",
      approvalRiskHintProtectedPath: "安全提示：目标位于受保护目录，继续执行前请确认本次操作符合你的预期。",
      approvalRiskHintSystemPathDeny: "安全提示：目标涉及系统关键路径，存在高风险，建议拒绝本次操作。",
      approvalRiskHintRuleAsk: "策略提示：命中询问规则，需你确认后才能继续执行。",
      approvalRiskHintDefaultAsk: "策略提示：未命中放行规则，系统默认要求人工确认。",
      stepPrefix: "步骤 {index}",
