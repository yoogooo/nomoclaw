...
export interface KnowledgeImportBatch { batchUid: string; knowledgeBaseUid: string; status: "DRAFT" | "BUILDING" | "COMPLETED" | "CANCELLED"; parserMode: string; chunkSizeTokens: number; chunkOverlapTokens: number; embeddingProviderId: string; embeddingModelId: string; embeddingDimension: number; items: KnowledgeImportItem[]; createdTime: string; updatedTime: string; }
...
export const knowledgeApi = {
  ...
  buildImport(uid: string, batchUid: string, payload: { parserMode: string; chunkSizeTokens: number; chunkOverlapTokens: number }) { return requestJson<KnowledgeImportBatch>(`/api/knowledge-bases/${uid}/imports/${batchUid}/build`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); },
