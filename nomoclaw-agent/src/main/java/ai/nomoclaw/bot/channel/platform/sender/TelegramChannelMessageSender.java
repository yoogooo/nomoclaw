...
@Slf4j
public class TelegramChannelMessageSender implements ChannelMessageSender {
    ...
    public TelegramChannelMessageSender(HttpClient appHttpClient, ChannelBotCredentialResolver credentialResolver) {
    ...
    public void send(OutboundMessage message) {
        ...
        ChannelBotCredentialResolver.TelegramBotCredential credential = credentialResolver.resolveTelegram(requestedBotId);
        if (credential == null || credential.token().isBlank()) {
            log.warn("[TelegramSender] missing bot token");
            return;
        }
        String chatId = message.address().target();
        if (chatId == null || chatId.isBlank()) {
            log.warn("[TelegramSender] empty chat_id");
            return;
        }
        ...
        for (int start = 0; start < text.length(); start += MAX_TEXT_LENGTH) {
            ...
            sendChunk(credential.token(), chatId, chunk);
        }
    }

    private void sendChunk(String token, String chatId, String text) {
        try {
            ...
            if (response.statusCode() >= 400) {
                log.warn("[TelegramSender] sendMessage failed status={} body={}", response.statusCode(), response.body());
            }
        } catch (Exception ex) {
            log.error("[TelegramSender] sendMessage error chatId={}", chatId, ex);
        }
    }
}
