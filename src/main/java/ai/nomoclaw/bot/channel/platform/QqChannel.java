...
public class QqChannel extends AbstractWebhookChannel implements Channel {

    private final ObjectMapper objectMapper = new ObjectMapper();

    public QqChannel(ChannelOrchestratorService orchestratorService,
                     AgentChannelsProperties properties,
                     HttpClient appHttpClient) {
        super(
                ChannelType.QQ,
                orchestratorService,
                new ChannelPolicy(properties.getQq().isRequireMention(), properties.getQq().allowSet()),
                appHttpClient
        );
    }
