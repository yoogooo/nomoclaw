...
CREATE TABLE IF NOT EXISTS agent_command_execution (
    ...
    command_text CLOB NOT NULL,
    cwd VARCHAR(1024) NOT NULL DEFAULT '',
    shell VARCHAR(64) NOT NULL DEFAULT '',
    exit_code INT NULL,
    success BOOLEAN NOT NULL DEFAULT FALSE,
    stdout_text CLOB NULL,
    stderr_text CLOB NULL,
    output_text CLOB NULL,
    error_code VARCHAR(64) NOT NULL DEFAULT '',
    error_message CLOB NULL,
    created_time TIMESTAMP(3) NOT NULL,
    PRIMARY KEY (id),
    CONSTRAINT uk_agent_command_execution_uid UNIQUE (execution_uid),
    INDEX idx_agent_command_execution_message (message_uid),
    INDEX idx_agent_command_execution_step (step_uid),
    INDEX idx_agent_command_execution_conversation (conversation_uid)
);
COMMENT ON TABLE agent_command_execution IS '命令执行明细日志表';
COMMENT ON COLUMN agent_command_execution.id IS '自增主键';
COMMENT ON COLUMN agent_command_execution.execution_uid IS '命令执行记录业务ID';
COMMENT ON COLUMN agent_command_execution.conversation_uid IS '所属对话ID';
COMMENT ON COLUMN agent_command_execution.message_uid IS '关联用户消息ID';
COMMENT ON COLUMN agent_command_execution.step_uid IS '关联步骤ID';
COMMENT ON COLUMN agent_command_execution.attempt IS '步骤执行尝试次数（从1开始）';
COMMENT ON COLUMN agent_command_execution.command_text IS '执行命令';
COMMENT ON COLUMN agent_command_execution.cwd IS '执行工作目录';
COMMENT ON COLUMN agent_command_execution.shell IS '执行 shell';
COMMENT ON COLUMN agent_command_execution.exit_code IS '命令退出码';
COMMENT ON COLUMN agent_command_execution.success IS '是否执行成功';
COMMENT ON COLUMN agent_command_execution.stdout_text IS '标准输出';
COMMENT ON COLUMN agent_command_execution.stderr_text IS '标准错误';
COMMENT ON COLUMN agent_command_execution.output_text IS '工具输出文本';
COMMENT ON COLUMN agent_command_execution.error_code IS '错误码';
COMMENT ON COLUMN agent_command_execution.error_message IS '错误信息';
COMMENT ON COLUMN agent_command_execution.created_time IS '创建时间';
